...
always @ (posedge clk_i or posedge rst_i) begin
    if(rst_i) begin
        ...
        case (cfg_araddr_i[7:0])
            ...
            `GPIO_INT_STATUS:
            begin
                data_r[`GPIO_INT_STATUS_RAW_R] <= gpio_int_status_raw_value;
            end
            `GPIO_INT_LEVEL:
            begin
...
wire [31:0] active_high_inputs = gpio_input_i & gpio_int_level_active_high_q;
wire [31:0] active_low_inputs = ~gpio_input_i & ~gpio_int_level_active_high_q;
wire [31:0] level_active_w = (active_high_inputs | active_low_inputs) & (~gpio_int_mode_edge_q);

// Edge detection logic
wire [31:0] edge_detect_w = input_last_q ^ gpio_input_i;
wire [31:0] rising_edges = edge_detect_w & gpio_input_i;
wire [31:0] falling_edges = edge_detect_w & ~gpio_input_i;
wire [31:0] edge_active_w = ((rising_edges & gpio_int_level_active_high_q) | 
                            (falling_edges & ~gpio_int_level_active_high_q)) & 
                            gpio_int_mode_edge_q;

// Interrupt status logic
// Interrupt clear signal
wire [31:0] interrupt_clear = (write_en_w && (cfg_awaddr_i[7:0] == `GPIO_INT_CLR)) ? 
                             cfg_wdata_i[`GPIO_INT_CLR_ACK_R] : 32'b0;

// Software interrupt set signal
wire [31:0] interrupt_set = (write_en_w && (cfg_awaddr_i[7:0] == `GPIO_INT_SET)) ? 
                           cfg_wdata_i[`GPIO_INT_SET_SW_IRQ_R] : 32'b0;

// Next state logic for raw interrupts
wire [31:0] next_interrupt_raw = (interrupt_raw_q |   // Clear takes priority
                                interrupt_set |                           // Software set
                                edge_active_w |                          // New edge triggers
                                level_active_w)& ~interrupt_clear;                          // Level triggers
                                
wire [31:0] raw_interrupt_value = next_interrupt_raw & gpio_int_mask_reg;

wire [31:0] intr_q_value = |(next_interrupt_raw & gpio_int_mask_reg);
...
always @(posedge clk_i or posedge rst_i) begin
    ...
    // Apply mask
        interrupt_raw_q <= raw_interrupt_value;
        intr_q <= intr_q_value;  
    end
end

// Output interrupts
assign gpio_int_status_raw_value = (interrupt_raw_q);
